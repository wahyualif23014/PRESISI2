...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _saveProfile() async {
    ...
    // TODO: Implementasi update ke API melalui AuthProvider/AuthService
          ...
          body: SingleChildScrollView(
            ...
            child: Column(
              children: [
                ...
                _buildProfileField(label: "Satuan Tugas", controller: _idTugasController, icon: Icons.account_balance_outlined, isReadOnly: true),
  ...
  Widget _buildUserHeader(UserModel? user) {
    return Column(
      children: [
        Text(user?.namaLengkap ?? "Pengguna", 
          style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.slate800)),
        Text(user?.roleDisplay.toUpperCase() ?? "UNKNOWN", 
          style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.greenPrimary, letterSpacing: 1.0)),
      ],
    );
  }
  ...
  Widget _buildProfileField({required String label, required TextEditingController controller, required IconData icon, bool isNumber = false, bool isReadOnly = false}) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isEnabled ? AppColors.greenPrimary : Colors.transparent, width: 1.5),
      ),
      child: TextFormField(
        ...
        decoration: InputDecoration(
          labelText: label, 
          labelStyle: TextStyle(color: isEnabled ? AppColors.greenPrimary : AppColors.slate400),
